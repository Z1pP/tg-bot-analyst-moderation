...
class TemplateCategoryRepository:
    ...
    async def get_categories_paginated(self, limit: int = 5, offset: int = 0) -> Sequence[TemplateCategory]:
        ...
        async with async_session() as session:
            try:
                ...
                categories = result.scalars().all()
                logger.info(f"Получено {len(categories)} категорий (страница {offset//limit + 1})")
                return categories
            except Exception as e:
    ...
    async def get_categories_count(self) -> int:
        ...
        async with async_session() as session:
            try:
                ...
                result = await session.execute(
                    select(func.count(TemplateCategory.id))
                )
                count = result.scalar()
