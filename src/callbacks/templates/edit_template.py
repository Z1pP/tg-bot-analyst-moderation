...
async def edit_template_callback(query: CallbackQuery, state: FSMContext) -> None:
...
async def edit_title_callback(query: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        await handle_exception(query.message, e, "edit_title_callback")
    finally:
        await query.answer()


@router.callback_query(F.data == "edit_content", TemplateStateManager.editing_template)
async def edit_content_callback(query: CallbackQuery, state: FSMContext) -> None:
...
async def cancel_edit_callback(query: CallbackQuery, state: FSMContext) -> None:
