...
async def toggle_target_status_handler(callback: CallbackQuery) -> None:
    ...
    try:
        ...
        if result:
            ...
            await callback.answer(f"Статус получателя {status_text}")
        else:
...
@router.callback_query(F.data.startswith("toggle_source__"))
async def toggle_source_status_handler(callback: CallbackQuery) -> None:
    """
    Обработчик для переключения статуса чата как источника данных.
    """
    try:
        # Получаем ID чата из callback_data
        chat_id = int(callback.data.split("__")[1])
        ...
        # Определяем новый статус (инвертируем текущий)
        new_status = not access.is_source if access else True

        # Устанавливаем новый статус
        result = await chat_tracking_repo.set_chat_as_source(
            admin_id=admin.id, chat_id=chat_id, is_source=new_status
        )
        ...
        if result:
            ...
            # Показываем уведомление
            status_text = "включен" if new_status else "отключен"
            await callback.answer(f"Статус источника {status_text}")
        else:
            await callback.answer("Не удалось изменить статус")

    except Exception as e:
        await handle_exception(callback.message, e, "toggle_source_status_handler")
        await callback.answer("Произошла ошибка")
