...
class NotifyArchiveChatMemberKickedUseCase:
    """
    UseCase для уведомления в архивный чат о кике непроверенного участника антиботом.
    """
    ...
    async def execute(self, dto: ArchiveMemberNotificationDTO) -> None:
        ...
        """
        logger.info(
            "Попытка уведомления о кике участника: chat_tgid=%s, user_tgid=%s, username=%s, chat_title=%s",
            dto.chat_tgid,
            dto.user_tgid,
            dto.username,
            dto.chat_title,
        ...
        if chat is None:
            logger.info(
                "Чат %s не найден в БД. Уведомление о кике участника %s пропущено.",
                dto.chat_tgid,
                dto.user_tgid,
            )
            return
        ...
        if not chat.archive_chat_id:
            logger.info(
                "Для чата %s (%s) не привязан архив. Уведомление о кике участника %s пропущено.",
                dto.chat_tgid,
                dto.chat_title,
                dto.user_tgid,
            )
        ...
        report_text = Dialog.ArchiveNotification.MEMBER_KICKED.format(
        ...
        logger.info(
            "Отправлено уведомление о кике участника %s в архивный чат %s для чата %s",
            dto.user_tgid,
