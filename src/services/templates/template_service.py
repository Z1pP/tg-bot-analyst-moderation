...
class TemplateService:
    ...
    async def get_global_templates_paginated(self, page: int = 1, page_size: int = 5) -> List[MessageTemplate]:
        """Получает глобальные шаблоны с пагинацией"""
        offset = (page - 1) * page_size
        return await self._template_repository.get_global_templates_paginated(offset=offset, limit=page_size)
    ...
    async def get_chat_templates_paginated(self, chat_id: int, page: int = 1, page_size: int = 5) -> List[MessageTemplate]:
        """Получает шаблоны для конкретного чата с пагинацией"""
        offset = (page - 1) * page_size
        return await self._template_repository.get_chat_templates_paginated(chat_id=chat_id, offset=offset, limit=page_size)
